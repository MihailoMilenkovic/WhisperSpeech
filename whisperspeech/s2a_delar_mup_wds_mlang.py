    ...
    def _trunc(samples):
        for s in samples:
            if random.random() < random_trunc_p:
                seconds = rand(0.3, 30)
                s["atoks.npy"] = s["atoks.npy"][
                    :, : math.ceil(seconds * atoks_per_second)
                ]
            s["stoks.npy"] = s["stoks.npy"][
                : math.ceil(s["atoks.npy"].shape[-1] / atoks_len * stoks_len)
            ]
            yield s
...
def pad_samples(atoks_len=2250, stoks_len=750, stoks_pad_token=4096):
    def _pad(samples):
        for s in samples:
            ...
            s["in_stoks"] = F.pad(
                stoks, (1, stoks_len - stoks.shape[-1] - 1), value=stoks_pad_token
            )
            q, n = atoks.shape
            padatoks = [F.pad(atoks[i], (i + 1, 0), value=1025) for i in range(q)]
            padatoks = [
                F.pad(padatoks[i], (0, atoks_len - n - i - 1), value=1024)
                for i in range(q)
            ]
...
class DelSumEmbedding(nn.Module):
    ...
    def forward(self, toks, xenc):
        with record_function("embeddings"):
            ...
            embs = torch.zeros(
                (b, newn, self.width), dtype=xenc.dtype, device=xenc.device
            )
...
class DelSumHead(nn.Module):
    ...
    def forward(self, x, embeddings=None):
        ...
        with record_function("unembed"):
            logits = torch.stack(
                [embeddings[q].unembed(split[:, :, q]) for q in range(self.quantizers)],
                dim=1,
            )
        return logits
...
class SADelARTransformer(nn.Module):
    def __init__(
        ...
        if stoks_width is None:
            stoks_width = width
        if spk_width is None:
            spk_width = width
        ...
        self.embds = DelSumEmbedding(
            pos_embs=self.positional_embeddings,
            length=ctx_n,
            n_head=n_head,
        ...
        self.head = DelSumHead(
            n_head=n_head, head_width=head_width, quantizers=quantizers
        )
    ...
    def load_frozen_acoustic_embeddings(self, amodel):
        for i in range(self.quantizers):
            self.decoder.embeddings[i].set_frozen_embeddings(
                amodel.quantizer.vq.layers[i].codebook
            )
    ...
    def _encoder(self, semb, positions):
        ...
        for l in self.encoder:
            x = l(x, positions, causal=self.tunables.causal_encoder)
    ...
    def run_encoder(self, Stoks, speakers):
        ...
        with record_function("encoder"):
            if self.positional_embeddings is not None:
                semb = semb + self.positional_embeddings
        ...
        if self.training and self.tunables.causal_encoder:
            enc_logits = (
                self.hidden_to_emb(xenc)
                @ self.semantic_embedding.weight.to(xenc.dtype).T
            ).float()
            enc_logits = (
        ...
        if self.spk_factor:
            spk_embs = self.spk_to_hidden(spk_embs)
    ...
    def forward(
        ...
        with record_function("loss"):
            ...
            if self.training:
                loss_denom += -1 + self.tunables.q0_loss_mult
            loss /= loss_denom
            if self.training and self.tunables.causal_encoder:
                loss += 0.1 * F.cross_entropy(
                    enc_logits[:, :-1].transpose(-1, -2), Stoks[:, 1:]
                )
        ...
        if not self.training:
            for i in range(self.quantizers):
                ...
                self.val_true[i] += (
                    (logits[:, i, :-1].argmax(-1)[valid_Atoks] == Atoks_i[valid_Atoks])
                    .float()
                    .sum()
                )
    ...
    def load_model(
        ...
        model = cls(
            **spec["config"], tunables=Tunables(**Tunables.upgrade(spec["tunables"]))
        )
    ...
    def optimize_training(self):
        ...
        self._encoder = torch.compile(
            self._encoder, fullgraph=True, mode="reduce-overhead"
        )
        ...
        with inference.inference_context():
            for i in it:
                with record_function("generate_one"):
                    toks[:, : i + 1, i + 1] = self.generate_next(
                        toks[:, :, i : i + 1],
...
def _make_model(
    size: str, quantizers: int = 4, tunables: Tunables = Tunables(), **kwargs
):
...
def make_model(
    ...
    frozen_acoustic_embeddings: bool = False,
    spk_width: int = None,
    ...
    vqmodel = (
        vq_stoks.RQBottleneckTransformer.load_model(frozen_embeddings_model)
