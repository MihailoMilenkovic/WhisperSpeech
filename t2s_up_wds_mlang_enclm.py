...
def ar_padder(ikey, okey, length, pad_token):
    ...
    def _ar_padder(samples):
        for s in samples:
            ...
            s["out_" + okey] = F.pad(
                toks, (0, length - toks.shape[-1]), value=pad_token
            )
            yield s
...
class Tunables:
    ...
    def __post_init__(self):
        ...
        if self.random:
            ...
            self.embeddings_std = 10 ** rand(-3, -0.7)
...
class T2SEmbedding(nn.Module):
    def __init__(
        self, length=1500, codes=1024, width=384, pos_embs=None, stoks_width=384
    ):
        super().__init__()
        self.embedding = FlexEmbeddings(
            codes, width, special_codes=1, frozen_width=stoks_width
        )
        if pos_embs is None:
            pos_embs = sinusoids(length, width)
        self.register_buffer("positional_embedding", pos_embs)

    def forward(self, Stoks, xenc, cps=None, offset=0):
        Sembs = self.embedding(Stoks)
        xin = (Sembs + self.positional_embedding[offset : offset + Sembs.shape[1]]).to(
            xenc.dtype
        )
        if cps is not None:
            xin = xin + cps
...
class Encoder(nn.Module):
    ...
    def forward(self, Stoks, positions, lang_emb=None):
        ...
        x = (xin + self.positional_embedding[positions]).to(xin.dtype)
        ...
        for l in self.layers:
            x = l(
                ...
                mask=(
                    self.mask
                    if self.tunables.causal_encoder and not self.training
                    else None
                ),
            )
...
class TSARTransformer(nn.Module):
    def __init__(
        ...
        self.tunables = tunables
        if self.stoks_width is None:
            self.stoks_width = self.width
        if self.ttoks_width is None:
            self.ttoks_width = self.width
        ...
        tformer_args = dict(
            width=width, n_head=n_head, ffn_mult=ffn_mult, tunables=tunables
        )
        self.encoder = Encoder(
            length=ttoks_len,
            codes=ttoks_codes,
            emb_width=self.ttoks_width,
            depth=encoder_depth,
            **tformer_args
        )
        self.embeddings = T2SEmbedding(
            length=stoks_len,
            codes=stoks_codes,
            width=width,
            stoks_width=self.stoks_width,
        )
    ...
    def load_frozen_semantic_embeddings(self, vqmodel):
        self.embeddings.embedding.set_frozen_embeddings(
            vqmodel.rq.layers[0]._codebook.embed[0]
        )
    ...
    def _embed_cps(self, cpss):
        if self.cps_embeddings is None:
            return None
    ...
    def run_encoder(self, in_ttoks, languages, cpss):
        if len(languages.shape) != 3:
            lang_embs = self.lang_embeddings(languages)
        else:
            lang_embs = languages
        if len(lang_embs.shape) == 2:
            lang_embs = lang_embs.unsqueeze(1)
    ...
    def forward(
        ...
        with record_function("decoder"):
            x = (
                self.embeddings.embedding(in_stoks)
        ...
        if loss is not None:
            with record_function("loss"):
                ...
                if self.training and self.tunables.causal_encoder:
                    ...
                    enc_logits = (
                        enc_logits
                        * self.tunables.output_mult
                        / (self.width / self.base_width)
                    )
                    loss += 0.1 * F.cross_entropy(
                        enc_logits.transpose(-1, -2), out_ttoks
                    )
    ...
    def load_model(
        cls,
        ref="collabora/whisperspeech:t2s-small-en+pl.model",
        repo_id=None,
        filename=None,
        local_filename=None,
        spec=None,
    ...
    def optimize(self, max_batch_size=1, dtype=torch.float16, torch_compile=True):
        ...
        if torch_compile:
            self.generate_next = torch.compile(
                self.generate_next, mode="reduce-overhead", fullgraph=True
            )
    ...
    def optimize_training(self):
        ...
        self.decoder = torch.compile(
            self.decoder, fullgraph=True, mode="reduce-overhead"
        )
    ...
    def generate_one(
        self, toks, toks_positions, cps_emb, xenc, xenc_positions, T, top_k
    ):
        probs, _ = self(
            None,
            None,
            ...
            None,
            toks,
            in_stoks_positions=toks_positions,
            loss=None,
            xenc=xenc,
            xenc_positions=xenc_positions,
            cps_emb=cps_emb,
        )
        probs = probs[:, -1]
    ...
    def prep(self, txt, cps=15, lang="en"):
        ...
        ttoks = F.pad(
            ttoks, (0, self.ttoks_len - len(ttoks)), value=self.tokenizer.eot
        ).unsqueeze(0)
    ...
    def generate(
        ...
        with inference.inference_context():
            for i in it:
                ...
                if i % 25 == 0 and (toks[:, i + 1] == self.stoks_codes - 1).all():
                    return toks[:, 1 : i + 1]
    ...
    def generate_batch(self, txts, N=None, T=1.1, top_k=7, show_progress_bar=True):
        ...
        for txt in txts:
            ...
            ttoks_ = F.pad(
                ttoks_, (0, self.ttoks_len - len(ttoks_)), value=self.tokenizer.eot
            ).unsqueeze(0)
            ttoks.append(ttoks_)
        ...
        for i in it:
            ...
            last_p = p[:, -1]
            if top_k:
                last_p[last_p < torch.topk(last_p, top_k).values[:, -1, None]] = (
                    -torch.inf
                )
