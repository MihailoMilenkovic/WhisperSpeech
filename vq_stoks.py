    ...
    def merge_loop(main_samples):
        ...
        for s in main_samples:
            ...
            try:
                ...
                merge_s = next(merged_samples)
            assert (
                merge_s["__key__"] == s["__key__"]
            ), f"sample keys don't match: {merge_s['__key__']}, {s['__key__']} in file {s['__url__']}"
            news = {}
            news.update(merge_s)
            news.update(s)
            yield news
...
def tokenize_text(samples, ttoks_size=200, model="base.en", language="en"):
    ...
    tokenizer = whisper.tokenizer.get_tokenizer(
        multilingual, language=language, task="transcribe"
    )
    for s in samples:
        ...
        s["out_ttoks"] = F.pad(
            torch.tensor(tokens[1:] + [tokenizer.eot]), (0, rpad), value=-100
        )
        yield s
...
class Tunables:
    ...
    rope: bool = True
    mask_embs: bool = (
        True  # force embeddings corresponding to the input audio padding to a constant value
    )
    ...
    def upgrade(args):
        ...
        if "encoder_depth_ratio" in args:
            del args["encoder_depth_ratio"]
        if "vq_codes" in args:
            del args["vq_codes"]
        return args
...
class RQBottleneckTransformer(nn.Module):
    def __init__(
        ...
        store_attr(
            "codebook_dim,vq_codes,q_depth,n_head,head_width,ffn_mult,depth,use_cosine_sim,downsample,whisper_model_name"
        )
        self.width = width
        ...
        if tunables.downsample_conv:
            self.downsample_conv = nn.Conv1d(
                width, width, kernel_size=3, stride=downsample, padding=1
            )
        else:
        ...
        self.positional_embedding = nn.Embedding(
            1500, width
        )  # FIXME: should be self.stoks_len
    ...
    def forward(self, samples, mask, input_toks, output_toks):
        ...
        if not self.training:
            ...
            self.val_true += (
                (logits.argmax(-1)[valid_toks] == output_toks[valid_toks]).float().sum()
            )
    ...
    def load_model(
        cls,
        ref="collabora/spear-tts-pytorch:whisper-vq-stoks-medium-en+pl.model",
        repo_id=None,
        filename=None,
        local_filename=None,
    ):
        ...
        vqmodel = cls(
            **spec["config"],
            tunables=Tunables(**Tunables.upgrade(spec.get("tunables", {}))),
        )
    ...
    def dequantize(self, stoks):
        ...
        if isinstance(stoks, np.ndarray):
    ...
    def encode_mel(self, mel):
        assert (
            len(mel.shape) == 3
        ), "invalid mel spectrogram shape, expect (batch,chn,time)"
        self.ensure_whisper()
        n = mel.shape[-1]
...
def make_model(
    size: str, tunables: Tunables = Tunables(), dataset: torch.utils.data.Dataset = None
):
    if size == "base.en-2d-4096c":
        model = RQBottleneckTransformer(
            codebook_dim=32,
            vq_codes=4096,
            q_depth=1,
            n_head=8,
